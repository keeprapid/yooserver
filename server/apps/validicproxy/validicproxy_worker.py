...
class ValidicProxy(threading.Thread, workers.WorkerBase):

    def __init__(self, moduleid):
        logr.debug("ValidicProxy :running in __init__")
        ...
        self.recv_queue_name = "W:Queue:ValidicProxy"
        if 'validicproxy' in self._config:
            if 'Consumer_Queue_Name' in _config['validicproxy']:
                self.recv_queue_name = _config['validicproxy']['Consumer_Queue_Name']
...
#            logr.debug('action_body : %s' % (action_body))
    ...
    def run(self):
        logr.debug("Start ValidicProxy pid=%s, threadindex = %s" % (os.getpid(),self.thread_index))
#        try:
        if 1:
            while 1:
...
if __name__ == "__main__":
    ...
    thread_count = 1
    if _config is not None and 'validicproxy' in _config and _config['validicproxy'] is not None:
        if 'thread_count' in _config['validicproxy'] and _config['validicproxy']['thread_count'] is not None:
            thread_count = int(_config['validicproxy']['thread_count'])

    for i in xrange(0, thread_count):
        memberlogic = ValidicProxy(i)
