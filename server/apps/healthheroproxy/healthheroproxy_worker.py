...
class HealthHeroProxy(threading.Thread, workers.WorkerBase):

    def __init__(self, moduleid):
        logr.debug("HealthHeroProxy :running in __init__")
        ...
        self.recv_queue_name = "W:Queue:HealthHeroProxy"
        if 'healthheroproxy' in self._config:
            if 'Consumer_Queue_Name' in _config['healthheroproxy']:
                self.recv_queue_name = _config['healthheroproxy']['Consumer_Queue_Name']
...
#            logr.debug('action_body : %s' % (action_body))
    ...
    def run(self):
        logr.debug("Start HealthHeroProxy pid=%s, threadindex = %s" % (os.getpid(),self.thread_index))
#        try:
        if 1:
            while 1:
...
if __name__ == "__main__":
    ...
    thread_count = 1
    if _config is not None and 'healthheroproxy' in _config and _config['healthheroproxy'] is not None:
        if 'thread_count' in _config['healthheroproxy'] and _config['healthheroproxy']['thread_count'] is not None:
            thread_count = int(_config['healthheroproxy']['thread_count'])

    for i in xrange(0, thread_count):
        memberlogic = HealthHeroProxy(i)
